...
/*
    ...
    man_args1.matmul_type = 8; // 6: 2x1; 7: 4x1, 8x1    
...
fp16* forward(Transformer* transformer, int token, int pos) {
    ...
    for(int l = 0; l < p->n_layers; l++) {
       ...
       /*printf("Check dma transf L1 e L2:\n");
        ...
        rmsnorm_parallelized_fp16(s->xb, x, BUFF4, buffer_n_cores, dim);
        // rmsnorm_parallelized_fp16(s->xb, x, w->rms_ffn_weight + l*dim, dim);
    ...
    rmsnorm_parallelized_fp16(s->xb, x, w->rms_final_weight, buffer_n_cores, dim);
